...
<template>
  <Transition 
    ...
    :style="{ transitionDelay: `${props.index * 100 + 200}ms` }"
  >
    <q-card class="my-card transform transition-transform duration-300 hover:-translate-y-1 group">
      <div class="overflow-hidden bg-neutral-900 dark:bg-neutral-700 hover:shadow-xl transition-all duration-300 rounded-lg border border-gray-200 dark:border-neutral-700">
        <div class="relative">
          <q-img 
            :src="gif.images?.original?.url" 
            :alt="gif.title || 'GIF sem título'" 
          ...
          <!-- Botão de favorito - mostra estado atual -->
          <button 
            @click.stop="toggleFavorite"
            class="absolute bottom-2 right-2 bg-white/90 dark:bg-gray-800/90 p-2 rounded-full transition-opacity duration-300 hover:bg-purple-100 dark:hover:bg-gray-700"
